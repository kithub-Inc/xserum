...
const Musics = () => {
    ...
    useEffect(() => { axios.get(`http://localhost:8080/musics/recently`).then(response => { setRecentlyData(response.data); }); }, []);
    useEffect(() => { axios.get(`http://localhost:8080/musics/popular`).then(response => { setPopularData(response.data); }); }, []);

    useEffect(() => { axios.get(`http://localhost:8080/musics/all`).then(response => { setAllData(response.data); }); }, []);
    ...
    return (
        <div className="main">
            <div className="container">
                <h1>💫&nbsp;&nbsp;<Search type="text" placeholder="Xserum Musics" ref={inputRef} /><SearchIco onClick={filterItem} src={search_ico} alt="search" /></h1>
...

                {
                    filterData.length === 0 ?
                    <>
                        <h2>💎 Recently Musics</h2>
                        <Presets short={true} data={recentlyData.slice(0, 6)} />
        
                        <h2>🎬 All Times Most Popular Musics</h2>
